...
# Priority: OTEL_EXPORTER_OTLP_ENDPOINT > MW_TARGET > fallback to localhost:4317
if [ -n "${OTEL_EXPORTER_OTLP_ENDPOINT:-}" ]; then
  # Use explicitly set OTEL_EXPORTER_OTLP_ENDPOINT (already set)
  : # No-op since variable is already set
elif [ -n "${MW_TARGET:-}" ]; then
  # Use MW_TARGET if OTEL_EXPORTER_OTLP_ENDPOINT is not set
  OTEL_EXPORTER_OTLP_ENDPOINT="${MW_TARGET}"
else
  # Fallback to localhost:4317
  OTEL_EXPORTER_OTLP_ENDPOINT="http://localhost:4317"
fi

# Priority: OTEL_EXPORTER_OTLP_HEADERS > MW_API_KEY > fallback to default
if [ -n "${OTEL_EXPORTER_OTLP_HEADERS:-}" ]; then
  # Use explicitly set OTEL_EXPORTER_OTLP_HEADERS (already set)
  : # No-op since variable is already set
elif [ -n "${MW_API_KEY:-}" ]; then
  # Use MW_API_KEY if OTEL_EXPORTER_OTLP_HEADERS is not set
  OTEL_EXPORTER_OTLP_HEADERS="authorization=${MW_API_KEY}"
else
  # Fallback to default
  OTEL_EXPORTER_OTLP_HEADERS="authorization=5xrocjh0p5ir233mvi34dvl5bepnyqri3rqb"
fi
...
detect_java_containers() {
  ...
  # Only echo if we have containers
  if [ ${#containers[@]} -gt 0 ]; then
    echo "${containers[@]}"
  fi
}

# Function to detect Tomcat servers and applications
detect_tomcat_apps() {
  local tomcat_apps=()
  
  # Look for Tomcat processes
  local tomcat_pids
  tomcat_pids=$(pgrep -f "tomcat\|catalina" 2>/dev/null || echo "")
    ...
    for pid in $tomcat_pids; do
      ...
      if echo "$cmdline" | grep -q "java.*tomcat\|java.*catalina"; then
        ...
        if [ -f "/proc/$pid/environ" ]; then
          catalina_home=$(cat "/proc/$pid/environ" 2>/dev/null | tr '\0' '\n' | grep "^CATALINA_HOME=" | cut -d'=' -f2- || echo "")
        fi
        
        # If CATALINA_HOME not found, try to extract from command line
        if [ -z "$catalina_home" ]; then
          # Look for -Dcatalina.home= in command line
          catalina_home=$(echo "$cmdline" | grep -o '\-Dcatalina\.home=[^[:space:]]*' | cut -d'=' -f2- || echo "")
        fi
        
        # If still not found, try common Tomcat locations
        if [ -z "$catalina_home" ]; then
          for common_path in "/opt/tomcat" "/usr/local/tomcat" "/var/lib/tomcat*" "/opt/apache-tomcat*"; do
            if [ -d "$common_path" ]; then
              catalina_home="$common_path"
              break
            fi
          done
        fi
        
        # Get Tomcat base directory (CATALINA_BASE)
        local catalina_base=""
        if [ -f "/proc/$pid/environ" ]; then
          catalina_base=$(cat "/proc/$pid/environ" 2>/dev/null | tr '\0' '\n' | grep "^CATALINA_BASE=" | cut -d'=' -f2- || echo "")
        fi
        
        if [ -z "$catalina_base" ]; then
          catalina_base=$(echo "$cmdline" | grep -o '\-Dcatalina\.base=[^[:space:]]*' | cut -d'=' -f2- || echo "")
        fi
        
        # Use CATALINA_HOME as fallback for CATALINA_BASE
        if [ -z "$catalina_base" ] && [ -n "$catalina_home" ]; then
  ...
  # Also check for systemd services that might be Tomcat
  for service in $(systemctl list-units --type=service --state=active --no-pager --no-legend | awk '{print $1}' | grep -E '\.(service)$'); do
    local exec_start
    exec_start=$(systemctl show "$service" --property=ExecStart --no-pager 2>/dev/null | cut -d'=' -f2- | tr -d '"')
    if echo "$exec_start" | grep -q "tomcat\|catalina"; then
      # Get the PID of this service
      local service_pid
      ...
      if [ -n "$service_pid" ] && [ "$service_pid" != "0" ]; then
        ...
        if ! printf '%s\n' "${tomcat_apps[@]}" | grep -q "tomcat-${service_pid}-"; then
          ...
          local catalina_home=""
          local catalina_base=""
          cmdline=$(cat "/proc/$service_pid/cmdline" 2>/dev/null | tr "\0" " " || echo "")
          if [ -n "$cmdline" ]; then
            catalina_home=$(echo "$cmdline" | grep -o "\-Dcatalina\.home=[^[:space:]]*" | cut -d"=" -f2- || echo "")
            catalina_base=$(echo "$cmdline" | grep -o "\-Dcatalina\.base=[^[:space:]]*" | cut -d"=" -f2- || echo "")
            if [ -z "$catalina_base" ] && [ -n "$catalina_home" ]; then
...
update_docker_containers() {
  ...
  # Get containers and handle empty results properly
  local java_containers_output
  java_containers_output=$(detect_java_containers)
  if [ -n "$java_containers_output" ]; then
    mapfile -t java_containers <<< "$java_containers_output"
  else
    java_containers=()
  fi
  ...
  # Check if docker-run-otel wrapper exists, install if missing
  if [ ! -f "$DOCKER_WRAPPER_PATH" ]; then
    info "Docker wrapper not found at $DOCKER_WRAPPER_PATH, installing it..."
    install_agent
...
update_docker_container() {
  ...
  info "Using endpoint for Docker container: ${OTEL_EXPORTER_OTLP_ENDPOINT}"
  info "Using headers for Docker container: ${OTEL_EXPORTER_OTLP_HEADERS}"
  info "Debug: MW_TARGET=${MW_TARGET:-not set}"
  info "Debug: OTEL_EXPORTER_OTLP_ENDPOINT=${OTEL_EXPORTER_OTLP_ENDPOINT}"
  ...
  docker_cmd="$docker_cmd -e OTEL_EXPORTER_OTLP_ENDPOINT=${OTEL_EXPORTER_OTLP_ENDPOINT}"
  docker_cmd="$docker_cmd -e OTEL_EXPORTER_OTLP_HEADERS=${OTEL_EXPORTER_OTLP_HEADERS}"
  docker_cmd="$docker_cmd -e OTEL_TRACES_EXPORTER=${OTEL_TRACES_EXPORTER:-otlp}"
  docker_cmd="$docker_cmd -e OTEL_METRICS_EXPORTER=${OTEL_METRICS_EXPORTER:-otlp}"
  docker_cmd="$docker_cmd -e OTEL_LOGS_EXPORTER=${OTEL_LOGS_EXPORTER:-otlp}"
  ...
  info "Debug: Environment variables being passed to docker-run-otel:"
  info "  OTEL_EXPORTER_OTLP_ENDPOINT=${OTEL_EXPORTER_OTLP_ENDPOINT}"
  info "  OTEL_EXPORTER_OTLP_HEADERS=${OTEL_EXPORTER_OTLP_HEADERS}"
...
update_java_services() {
  ...
  for service in "${java_services[@]}"; do
    ...
    if update_service_config "$service"; then
      info "✓ Successfully updated service: $service"
    else
      warn "✗ Failed to update service: $service"
    fi
  done
}

# Function to update existing Tomcat applications
update_tomcat_apps() {
  if [ "$AUTO_UPDATE_SERVICES" != "1" ]; then
    info "AUTO_UPDATE_SERVICES is disabled, skipping Tomcat app updates"
    return 0
  fi
  
  info "Detecting existing Tomcat applications..."
  local tomcat_apps
  mapfile -t tomcat_apps < <(detect_tomcat_apps)
  
  if [ ${#tomcat_apps[@]} -eq 0 ]; then
    info "No running Tomcat applications detected"
    return 0
  fi
  
  info "Found Tomcat applications: ${#tomcat_apps[@]}"
...
# Function to update a specific Tomcat application
update_tomcat_app() {
  local app_id="$1"
  local pid="$2"
  local catalina_home="$3"
  local catalina_base="$4"
  local app_name="$5"
  local app_path="$6"
  
  info "Updating Tomcat application: $app_name (PID: $pid)"
  ...
  if [ -n "$setenv_file" ]; then
    ...
    if [ -f "$setenv_file" ]; then
      cp "$setenv_file" "$backup_dir/setenv.sh.backup"
...
if [ -z "\${JAVA_OPTS:-}" ]; then
  ...
  # Check if agent is already in JAVA_OPTS
  if echo "\$JAVA_OPTS" | grep -q "javaagent:${AGENT_PATH}"; then
    # Agent already present, do nothing
    :
  else
    # Add agent to beginning of JAVA_OPTS
    export JAVA_OPTS="-javaagent:${AGENT_PATH} \$JAVA_OPTS"
  fi
fi

# Add JVM options to prevent cgroup-related issues and runtime metrics failures
export JAVA_OPTS="\$JAVA_OPTS -Dotel.javaagent.enable.runtime.metrics=false"
...
EOF
  ...
  if [ -n "$service_name" ]; then
    ...
    # For standalone Tomcat, we need to restart it to pick up the new setenv.sh
    info "Tomcat is not managed by systemd, manual restart required"
    info "Please restart Tomcat to apply OpenTelemetry instrumentation:"
    info "  - Stop: kill $pid"
    info "  - Start: $catalina_script start"
    warn "Manual restart required for Tomcat application: $app_name"
    warn "Note: The script will continue but Tomcat instrumentation will not be active until manual restart"
  fi
}

# Function to instrument a specific Tomcat application
instrument_tomcat_app() {
  local app_id="$1"
  
  if [ -z "$app_id" ]; then
    err "Tomcat application ID is required"
    return 1
  fi
  ...
  # Find the Tomcat app details
  local tomcat_apps
  mapfile -t tomcat_apps < <(detect_tomcat_apps)
  
  local app_info=""
  for app in "${tomcat_apps[@]}"; do
    if [[ "$app" == "$app_id|"* ]]; then
      app_info="$app"
      break
    fi
  ...
  if [ -z "$app_info" ]; then
    err "Tomcat application $app_id not found or not running"
    return 1
  fi
...
update_service_config() {
  ...
  if systemctl is-active --quiet "$service"; then
    info "Restarting service: $service (this may take a moment)..."
    # Add timeout to prevent hanging and redirect output to avoid terminal interference
    if timeout 30 systemctl restart "$service" >/dev/null 2>&1; then
      info "✓ Service $service restarted successfully"
    else
      warn "Service $service restart timed out or failed"
      # Check if service is still running
      if systemctl is-active --quiet "$service"; then
        info "Service $service is still running despite timeout"
      else
        warn "Service $service may have failed to start"
      fi
    fi
  fi
}
...
instrument_service() {
  ...
  systemctl daemon-reload
  # Add timeout to prevent hanging and redirect output to avoid terminal interference
  if timeout 30 systemctl restart "$service_name" >/dev/null 2>&1; then
    info "✓ Service $service_name restarted successfully"
  else
    warn "Service $service_name restart timed out or failed"
    # Check if service is still running
    if systemctl is-active --quiet "$service_name"; then
      info "Service $service_name is still running despite timeout"
    else
      warn "Service $service_name may have failed to start"
    fi
  fi
...
list_instrumented_apps() {
  ...
  # Check Tomcat applications
  info "=== Tomcat Applications ==="
  local tomcat_apps
  mapfile -t tomcat_apps < <(detect_tomcat_apps)
  ...
  for app_info in "${tomcat_apps[@]}"; do
    ...
    if [ -n "$env_output" ]; then
      ...
      info "✗ Tomcat App: $app_name (PID: $pid) (not instrumented) - App ID: $app_id"
    fi
  done
...
# Function to remove OTEL instrumentation from a specific Tomcat application
uninstrument_tomcat_app() {
  local app_id="$1"
...
# Function to install system-wide binary
install_binary() {
  local binary_name="mw-instrument"
  ...
  cat > "$binary_path" <<EOF
#!/usr/bin/env bash
# mw-instrument - Middleware Java Instrumentation Tool
# This is a system-wide binary wrapper for the instrumentation script

# Find the original script (look in common locations)
ORIGINAL_SCRIPT=""
for path in "$script_path" \
            "/opt/middleware/scripts/auto-instrumentation/instrument-java-so-stable-tomcat.sh" \
            "/usr/local/share/middleware/scripts/auto-instrumentation/instrument-java-so-stable-tomcat.sh" \
            "/usr/local/bin/instrument-java-so-stable-tomcat.sh" \
            "\$(which instrument-java-so-stable-tomcat.sh 2>/dev/null)"; do
  if [ -f "\$path" ]; then
...
if [ -z "\$ORIGINAL_SCRIPT" ]; then
  echo "ERROR: Could not find the original instrumentation script" >&2
  echo "Please ensure the script is in one of these locations:" >&2
  echo "  - $script_path" >&2
  echo "  - /opt/middleware/scripts/auto-instrumentation/instrument-java-so-stable-tomcat.sh" >&2
  echo "  - /usr/local/share/middleware/scripts/auto-instrumentation/instrument-java-so-stable-tomcat.sh" >&2
  echo "  - /usr/local/bin/instrument-java-so-stable-tomcat.sh" >&2
  exit 1
fi
...
EOF
  ...
  info "You can now use: $binary_name <command>"
}

# Function to remove system-wide binary
uninstall_binary() {
  local binary_name="mw-instrument"
  local binary_path="/usr/local/bin/$binary_name"
  
  if [ -f "$binary_path" ]; then
    info "Removing system-wide binary: $binary_name"
    rm -f "$binary_path"
...
uninstrument_all() {
  ...
  # Remove from Tomcat applications
  info "=== Removing from Tomcat Applications ==="
  local tomcat_apps
  mapfile -t tomcat_apps < <(detect_tomcat_apps)
  ...
  for app_info in "${tomcat_apps[@]}"; do
    ...
    env_output=$(cat "/proc/$pid/environ" 2>/dev/null | tr '\0' '\n' | grep -E "OTEL_EXPORTER_OTLP_ENDPOINT" || echo "")
    if [ -n "$env_output" ]; then
      info "Removing instrumentation from Tomcat app: $app_name"
      if uninstrument_tomcat_app "$app_id"; then
        total_removed=$((total_removed + 1))
      fi
    else
      info "Tomcat app $app_name is not instrumented, skipping"
    fi
  done
...
validate_instrumentation() {
  ...
  local total_apps=0
  local instrumented_apps=0
  
  # Check systemd services
  local java_services
  mapfile -t java_services < <(detect_java_services)
  total_apps=$((total_apps + ${#java_services[@]}))
  ...
  for service in "${java_services[@]}"; do
    ...
    if echo "$env_output" | grep -q "OTEL_EXPORTER_OTLP_ENDPOINT"; then
      ...
      instrumented_apps=$((instrumented_apps + 1))
    else
  ...
  total_apps=$((total_apps + ${#java_containers[@]}))
  ...
  for container in "${java_containers[@]}"; do
    ...
    has_endpoint=$(docker exec "$container" env 2>/dev/null | grep -q "OTEL_EXPORTER_OTLP_ENDPOINT" && echo "yes" || echo "no")
    if [ "$has_endpoint" = "yes" ]; then
      info "✓ Container $container has OTEL configuration"
      instrumented_apps=$((instrumented_apps + 1))
    else
      warn "✗ Container $container missing OTEL configuration"
    fi
  done
  ...
  local tomcat_apps
  mapfile -t tomcat_apps < <(detect_tomcat_apps)
  total_apps=$((total_apps + ${#tomcat_apps[@]}))
  
  for app_info in "${tomcat_apps[@]}"; do
    IFS='|' read -r app_id pid catalina_home catalina_base app_name app_path <<< "$app_info"
    info "Checking Tomcat app: $app_name (PID: $pid)"
    
    local env_output
    env_output=$(cat "/proc/$pid/environ" 2>/dev/null | tr '\0' '\n' | grep -E "OTEL_EXPORTER_OTLP_ENDPOINT" || echo "")
    if [ -n "$env_output" ]; then
      info "✓ Tomcat app $app_name has OTEL configuration"
      instrumented_apps=$((instrumented_apps + 1))
    else
      warn "✗ Tomcat app $app_name missing OTEL configuration"
    fi
  done
  
  info "Total Java applications found: $total_apps"
  info "Total instrumented applications: $instrumented_apps"
  
  if [ $instrumented_apps -eq $total_apps ] && [ $total_apps -gt 0 ]; then
    info "✓ All Java applications are properly instrumented"
    return 0
  elif [ $total_apps -eq 0 ]; then
    info "No Java applications found to instrument"
    return 0
  else
    warn "Some applications may not be properly instrumented"
    return 1
  fi
}
...
if [ "$EUID" -ne 0 ]; then
  case "${1:-}" in
    install-binary|install-agent|install-host|docker-wrapper|patch-k8s|\
    update-services|update-docker|update-tomcat|all|instrument-all|instrument-service|instrument-container|instrument-tomcat|\
    uninstrument-all|uninstrument-service|uninstrument-container|uninstrument-tomcat|uninstall-binary)
      # These operations require root privileges, re-run with sudo
...
install_systemd_dropin() {
  ...
  info "Using endpoint for systemd drop-in: ${OTEL_EXPORTER_OTLP_ENDPOINT}"
  info "Using headers for systemd drop-in: ${OTEL_EXPORTER_OTLP_HEADERS}"
...
# Apply the same fallback logic for OTEL_EXPORTER_OTLP_ENDPOINT
# Priority: OTEL_EXPORTER_OTLP_ENDPOINT > MW_TARGET > fallback to localhost:4317
echo "Docker wrapper debug: OTEL_EXPORTER_OTLP_ENDPOINT=${OTEL_EXPORTER_OTLP_ENDPOINT:-not set}"
echo "Docker wrapper debug: MW_TARGET=${MW_TARGET:-not set}"
if [ -n "${OTEL_EXPORTER_OTLP_ENDPOINT:-}" ]; then
  # Use explicitly set OTEL_EXPORTER_OTLP_ENDPOINT
  FINAL_OTEL_ENDPOINT="${OTEL_EXPORTER_OTLP_ENDPOINT}"
  echo "Docker wrapper: Using explicit OTEL_EXPORTER_OTLP_ENDPOINT=${FINAL_OTEL_ENDPOINT}"
elif [ -n "${MW_TARGET:-}" ]; then
  # Use MW_TARGET if OTEL_EXPORTER_OTLP_ENDPOINT is not set
  FINAL_OTEL_ENDPOINT="${MW_TARGET}"
  echo "Docker wrapper: Using MW_TARGET fallback=${FINAL_OTEL_ENDPOINT}"
else
  # Fallback to localhost:4317
  FINAL_OTEL_ENDPOINT="http://localhost:4317"
  echo "Docker wrapper: Using localhost fallback=${FINAL_OTEL_ENDPOINT}"
fi

# Apply the same fallback logic for OTEL_EXPORTER_OTLP_HEADERS
# Priority: OTEL_EXPORTER_OTLP_HEADERS > MW_API_KEY > fallback to default
if [ -n "${OTEL_EXPORTER_OTLP_HEADERS:-}" ]; then
  # Use explicitly set OTEL_EXPORTER_OTLP_HEADERS
  FINAL_OTEL_HEADERS="${OTEL_EXPORTER_OTLP_HEADERS}"
elif [ -n "${MW_API_KEY:-}" ]; then
  # Use MW_API_KEY if OTEL_EXPORTER_OTLP_HEADERS is not set
  FINAL_OTEL_HEADERS="authorization=${MW_API_KEY}"
else
  # Fallback to default
  FINAL_OTEL_HEADERS="authorization=5xrocjh0p5ir233mvi34dvl5bepnyqri3rqb"
fi

# Add OpenTelemetry environment variables
args+=( -e "OTEL_EXPORTER_OTLP_ENDPOINT=${FINAL_OTEL_ENDPOINT}" )
args+=( -e "OTEL_EXPORTER_OTLP_HEADERS=${FINAL_OTEL_HEADERS}" )
...
Commands:
  install-binary      Install system-wide binary "mw-instrument" for easy access
  install-agent       Download/install the Java agent to $AGENT_PATH
  install-host        (install-agent + systemd drop-in + profile snippet + update services + update docker + update tomcat + validate)
  ...
  instrument-tomcat <app-id>   Add OTEL instrumentation to specific Tomcat application
  uninstrument-all    Remove OTEL instrumentation from all Java apps (services, containers, and Tomcat)
...
Environment Variables:
  ...
  MW_TARGET                      Alternative endpoint variable (used if OTEL_EXPORTER_OTLP_ENDPOINT not set)
  OTEL_EXPORTER_OTLP_HEADERS    Authentication headers (priority: OTEL_EXPORTER_OTLP_HEADERS > MW_API_KEY > default)
  MW_API_KEY                     API key for authorization (used if OTEL_EXPORTER_OTLP_HEADERS not set)
...
Examples:
  # First time setup - install binary for easy access
  ...
  sudo $0 update-tomcat         # Update existing Tomcat applications only
  ...
  sudo $0 instrument-tomcat tomcat-1234-myapp  # Instrument specific Tomcat application
...
Tomcat Application IDs:
  Tomcat applications are identified by format: tomcat-<pid>-<app-name>
  Use 'list-instrumented' to see available Tomcat application IDs
EOF
}
...
main() {
  ...
  case "${1:-}" in
    ...
    update-tomcat) update_tomcat_apps ;;
    ...
    instrument-tomcat)
      if [ -z "$2" ]; then
        err "Tomcat application ID is required for instrument-tomcat"
        # shellcheck disable=SC2317
        usage
        # shellcheck disable=SC2317
        exit 1
      fi
      instrument_tomcat_app "$2" ;;
    ...
    uninstrument-tomcat)
      if [ -z "$2" ]; then
        err "Tomcat application ID is required for uninstrument-tomcat"
        # shellcheck disable=SC2317
        usage
        # shellcheck disable=SC2317
        exit 1
    ...
    "")
      ...
      if [ -f "/usr/local/bin/mw-instrument" ]; then
        # Binary already exists, show help
        usage
      else
        # Binary not installed, install it
        install_binary
      fi
      ;;
    help|-h|--help) usage ;;
