...
for path in "$script_path" \
            "/opt/middleware/scripts/auto-instrumentation/instrument-java-so-stable.sh" \
            "/usr/local/share/middleware/scripts/auto-instrumentation/instrument-java-so-stable.sh" \
            "/usr/local/bin/instrument-java-so-stable.sh" \
            "\$(which instrument-java-so-stable.sh 2>/dev/null)"; do
  if [ -f "\$path" ]; then
...
if [ -z "\$ORIGINAL_SCRIPT" ]; then
  ...
  echo "Please ensure the script is installed in one of the expected locations:" >&2
  echo "  - $script_path" >&2
  echo "  - /opt/middleware/scripts/auto-instrumentation/instrument-java-so-stable.sh" >&2
  echo "  - /usr/local/share/middleware/scripts/auto-instrumentation/instrument-java-so-stable.sh" >&2
  echo "  - /usr/local/bin/instrument-java-so-stable.sh" >&2
  echo "  - Or install it in your PATH" >&2
  exit 1
fi
...
EOF
  ...
  info "✓ You can now run: $binary_name <command>"
  info "✓ Example: $binary_name install-host"
}
...
# Intelligent privilege check - re-run with sudo only if needed
# Operations that require root: install-*, update-*, instrument-*, uninstrument-*
# Operations that don't require root: list-instrumented, validate, help
if [ "$EUID" -ne 0 ]; then
  case "${1:-}" in
    install-binary|install-agent|install-host|docker-wrapper|patch-k8s|\
    update-services|update-docker|all|instrument-service|instrument-container|\
    uninstrument-all|uninstrument-service|uninstrument-container|uninstall-binary)
      ...
      exec sudo -E "$0" "$@"
      ;;
    list-instrumented|validate|""|help|-h|--help)
      # These operations can run without root
      ;;
    *)
      # Unknown command, let it proceed (will show usage anyway)
      ;;
  esac
fi
  ...
  case "${1:-}" in
    ...
    "")
      ...
      if [ -f "/usr/local/bin/mw-instrument" ]; then
        ...
        info "No command provided. Installing system-wide binary for easy access..."
        install_binary
        info ""
        info "Binary installed! You can now use: mw-instrument <command>"
        info "Run 'mw-instrument help' to see available commands."
      fi
      ;;
    help|-h|--help) usage ;;
    *)
