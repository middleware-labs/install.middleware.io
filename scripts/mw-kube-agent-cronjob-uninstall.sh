#!/bin/sh
set -e errexit
LOG_FILE="/var/log/mw-kube-agent/mw-kube-agent-cronjob-uninstall-$(date +%s).log"
...
function on_exit {
  if [ $? -eq 0 ]; then
    send_logs "installed" "cronjob uninstallation Completed"
  else
    send_logs "error" "cronjob uninstallation failed"
  fi
}
...
echo -e "\nUninstalling CronJob for upgrading Middleware Kubernetes agent ...\n\n\tcluster : $MW_KUBE_CLUSTER_NAME \n\tcontext : $CURRENT_CONTEXT\n"
...
if [ -z "${MW_KUBECONFIG}" ]; then
  kubectl delete --kubeconfig=${MW_KUBECONFIG}  -f $MW_KUBE_AGENT_HOME/cronjob.yaml
else  
  kubectl delete -f $MW_KUBE_AGENT_HOME/cronjob.yaml
fi

echo "Middleware Kubernetes agent CronJob successfully uninstalled !"
