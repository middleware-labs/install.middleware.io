...
# get_latest_mw_injector_version() {
#   repo="middleware-labs/java-injector"
...
# if [ "${MW_JAVA_AUTO_INSTRUMENT}" = "false" ]; then
#   echo "MW_JAVA_AUTO_INSTRUMENT is set to false. Skipping Java auto-instrumentation."
#   exit 0
# fi

# # Get MW Injector version
# # MW_INJECTOR_LATEST_VERSION=$(get_latest_mw_injector_version)
# # if [ "${MW_INJECTOR_VERSION}" = "" ]; then
# #   MW_INJECTOR_VERSION=$MW_INJECTOR_LATEST_VERSION
# # fi
...
# INJECTOR_ARCH=""
# case "$MW_DETECTED_ARCH" in
#   amd64|i386|i486|i586|i686|x32)
#     INJECTOR_ARCH="amd64"
#     ;;
#   arm64|armhf|armel|armeb)
#     INJECTOR_ARCH="arm64"
#     ;;
#   *)
#     echo "Warning: Unsupported architecture: $MW_DETECTED_ARCH"
...
# # Download MW Injector binary
# echo "Downloading MW Injector binary..."
# INJECTOR_BINARY_URL="https://github.com/middleware-labs/java-injector/releases/download/${MW_INJECTOR_VERSION}/mw-injector-linux-${INJECTOR_ARCH}"
...
# echo "Architecture: ${INJECTOR_ARCH}"
# echo "Binary Name: ${INJECTOR_BINARY_NAME}"
...
# if ! sudo curl -L -o "$INJECTOR_BINARY_PATH" "$INJECTOR_BINARY_URL"; then
#   echo "Error: Failed to download MW Injector binary from $INJECTOR_BINARY_URL"
#   exit 1
# fi
...
# JAVA_AGENT_URL="https://github.com/middleware-labs/opentelemetry-java-instrumentation/releases/download/${MW_JAVA_AGENT_VERSION}/${JAVA_AGENT_JAR}"
# JAVA_AGENT_PATH="${MW_JAVA_AGENT_DIR}/${JAVA_AGENT_JAR}"

# if ! sudo curl -L -o "$JAVA_AGENT_PATH" "$JAVA_AGENT_URL"; then
#   echo "Error: Failed to download Java Agent from $JAVA_AGENT_URL"
#   exit 1
# fi
...
# # Check for Java processes (systemd)
...
#   JAVA_PROCESS_COUNT=$(grep -c "PID:" /tmp/mw-java-processes.txt || echo "0")

#   if [ "$JAVA_PROCESS_COUNT" -gt 0 ]; then
#     echo -e "\n📋 Found $JAVA_PROCESS_COUNT Java process(es):\n"
...
#         # Run auto-instrument with environment variables piped in
#         (
#           source "$ENV_FILE"
#           echo "$MW_API_KEY"
#           echo "$MW_TARGET"
#           echo "$JAVA_AGENT_PATH"
#         ) | sudo -E "$INJECTOR_BINARY_PATH" auto-instrument-config
...
#   fi
# else
#   echo "⚠️  Could not scan for Java processes (this is normal if Java is not installed)"
# fi
...
#     CONTAINER_COUNT=$(grep -c "Container:" /tmp/mw-java-containers.txt || echo "0")

#     if [ "$CONTAINER_COUNT" -gt 0 ]; then
#       echo -e "\n🐳 Found $CONTAINER_COUNT Java Docker container(s):\n"
...
#           # Run docker instrumentation with environment variables piped in
#           (
#             source "$ENV_FILE"
#             echo "$MW_API_KEY"
#             echo "$MW_TARGET"
#             echo "$JAVA_AGENT_PATH"
#           ) | sudo -E "$INJECTOR_BINARY_PATH" instrument-docker
...
# echo "  sudo mw-injector list                    # List Java processes"
# echo "  sudo mw-injector list-docker             # List Docker containers"
# echo "  sudo mw-injector auto-instrument         # Instrument host processes"
# echo "  sudo mw-injector instrument-docker       # Instrument all Docker containers"
# echo ""
# echo "For more commands, run: mw-injector --help"
# echo "========================================="
#!/bin/bash
