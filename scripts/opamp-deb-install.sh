...
if [ "${MW_REMOTE_MANAGEMENT}" = "" ]; then
  MW_REMOTE_MANAGEMENT=false
fi
export MW_REMOTE_MANAGEMENT
...
if [ "${MW_AGENT_GROUP}" = "" ]; then
  MW_AGENT_GROUP="default"
fi
export MW_AGENT_GROUP

if [ "${MW_OPAMP_SERVER_URL}" = "" ]; then
  MW_OPAMP_SERVER_URL="ws://localhost:4320/v1/opamp"
fi
export MW_OPAMP_SERVER_URL
...
# Enable and start appropriate service based on mode
if [ "${MW_REMOTE_MANAGEMENT}" = "true" ]; then
    echo "Enabling Middleware Agent in Remote Management mode (OpAMP)..."
    
    if ! sudo systemctl enable mw-agent-opamp; then
      echo "Error: Failed to enable mw-agent-opamp service."
      exit 1
    fi
    
    if [ "${MW_AUTO_START}" = true ]; then
        sudo systemctl start mw-agent-opamp
        echo "Middleware Agent (OpAMP managed) started successfully."
    fi
else
    echo "Enabling Middleware Agent in Standalone mode..."
    
    if ! sudo systemctl enable mw-agent; then
      echo "Error: Failed to enable mw-agent service."
      exit 1
    fi
    
    if [ "${MW_AUTO_START}" = true ]; then
        sudo systemctl start mw-agent
        echo "Middleware Agent (standalone) started successfully."
    fi
fi
...
if [ "${MW_REMOTE_MANAGEMENT}" = "true" ]; then
    echo -e "\nMiddleware Agent installation completed successfully."
    echo "Running in: Remote Management Mode (OpAMP)"
    echo "Service: mw-agent-opamp"
    echo "View logs: sudo journalctl -u mw-agent-opamp -f"
else
    echo -e "\nMiddleware Agent installation completed successfully."
    echo "Running in: Standalone Mode"
    echo "Service: mw-agent"
    echo "View logs: sudo journalctl -u mw-agent -f"
fi
