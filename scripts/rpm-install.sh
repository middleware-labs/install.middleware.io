...
if [ "${MW_ENABLE_OBI}" = true ]; then
  ...
  if ! run_cmd bash -c "$(curl -fsSL https://install.middleware.io/scripts/install-obi.sh)"; then
    ...
    log_warn "OBI Agent installation did not complete — detected kernel ${DETECTED_KERNEL} does not meet OBI's minimum requirement (5.8+, or 4.18+ on RHEL/CentOS). Continuing without it. Core mw-agent and OTel Injector monitoring are unaffected. See https://opentelemetry.io/docs/zero-code/obi/ for OBI kernel requirements."
  fi
else
