...
sudo touch /etc/yum.repos.d/mw-agent.repo
if [[ $MW_DETECTED_ARCH == "x86_64" ]]; then
sudo cat > /etc/yum.repos.d/mw-agent.repo <<EOF
name=Middleware Agent
baseurl=https://raw.githubusercontent.com/middleware-labs/yum.middleware.io/master/x86_64/
enabled=1
gpgcheck=0
EOF
...
sudo cat > /etc/yum.repos.d/mw-agent.repo <<EOF
name=Middleware Agent
baseurl=https://raw.githubusercontent.com/middleware-labs/yum.middleware.io/master/aarch64/
enabled=1
gpgcheck=0
EOF
fi
