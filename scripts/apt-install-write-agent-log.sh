...
#!/bin/sh
cd /usr/bin && (MW_API_KEY=$MW_API_KEY TARGET=$TARGET mw-go-agent-host start > /usr/local/bin/mw-go-agent/mw-agent.log)
EOIF
else 
cat << EOELSE > /usr/local/bin/mw-go-agent/apt/executable
#!/bin/sh
cd /usr/bin && (MW_API_KEY=$MW_API_KEY mw-go-agent-host start > /usr/local/bin/mw-go-agent/mw-agent.log)
EOELSE
fi
