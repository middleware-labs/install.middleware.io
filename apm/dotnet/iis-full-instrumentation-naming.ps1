...
foreach ($AppPoolName in $SelectedAppPools) {
    ...
    # Service name defaults to the App Pool's own name so each pool is identifiable in traces
    $envs = $baseEnvs.Clone()
    $envs["OTEL_SERVICE_NAME"] = $AppPoolName
    ...
    $appPoolEnvVarsToRemove = ($oldEnvVars + $baseEnvs.Keys + "OTEL_SERVICE_NAME") | Sort-Object -Unique
