...
function Download-OpenTelemetry([string]$Version, [string]$Path) {
    $archive = "middleware-dotnet-instrumentation-windows.zip"
    $RELEASES_URL = "https://install.middleware.io/apm/dotnet"
    $dlUrl = "$RELEASES_URL/$Version/$archive"
...
function Get-Environment-Variables-Table([string]$InstallDir, [string]$OTelServiceName) {
    ...
    $OTEL_DOTNET_AUTO_PLUGINS = "Middleware.ObservabilityPlugin, Middleware, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null"
    if (-not $env:MW_AGENT_SERVICE) {
        $env:MW_AGENT_SERVICE = "localhost"
    }
    $OTEL_EXPORTER_OTLP_ENDPOINT = "http://${env:MW_AGENT_SERVICE}:9320"
